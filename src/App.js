...
export const App = () => {
  ...
  const handleChange = (event) => {
    ...
    if (event.target.value === "incomplete") {
      ...
      const completeTodoList = [...todoList].filter((todo) => todo.status === "未着手");
      setFilteredTodoList(completeTodoList);
    } return
  }
  ...
  const onClickSwitch = (index) => {
    ...
    if (switchTodoList[index].status === "未着手") {
      switchTodoList[index].status = "作業中";
    } else if (switchTodoList[index].status === "作業中") {
      switchTodoList[index].status = "完了";
    } else if (switchTodoList[index].status === "完了") {
      switchTodoList[index].status = "未着手";
    }
  ...
  return (
    <>
      <div className="complete-area">
        ...
        <label>
          <input type="radio" value="created" onChange={handleChange} checked={radio === 'created'} />
          未着手
        </label>
